<template>
  <section>
    <div class="column">
      <nav class="breadcrumb" aria-label="breadcrumbs">
        <ul>
          ...
          <li class="is-active">
            <a href="#" aria-current="page">Job List</a>
          </li>
        </ul>
      </nav>
    </div>
    <div class="column card">
      ...
      <router-link class="button is-success" to="/admin/jobs/create">
        New Job
      </router-link>
...
export default {
  ...
  methods: {
    async getJobs() {
      try {
        await this.$store.dispatch('job/getJobs')
      } catch (err) {
        console.log(err)
      }
    },
    async deleteJob(payload) {
      ...
      if (payload) {
        try {
          await this.$store.dispatch('job/deleteJob', payload.id)
          this.getJobs()
        } catch (err) {
          console.log(err)
        }
