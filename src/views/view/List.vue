  ...
  <section>
    <div class="column">
          ...
          <b-table-column field="actions" label="Actions" width="90" centered>
            <div class="buttons">
              <router-link
                ...
                :to="`/admin/schemas/${schemaID}/views/${props.row.id}`"
              >
      ...
      <router-link
        ...
        :to="`/admin/schemas/${schemaID}/views/create`"
      >
        Create View
      </router-link>
      <b-modal :active.sync="isModalDelete" has-modal-card>
        <modal-yes-no
          ...
          :data="{ schema_id: schemaID, view_id: viewID }"
          :method="deleteView"
        ></modal-yes-no>
      </b-modal>
      <b-loading
...
export default {
  ...
  watch: {
    dataStore() {
      this.data = this.$store.getters['view/views']
    },
  },
  mounted() {
    this.getViews(this.schemaID)
  },
  methods: {
    async getViews(schemaID) {
      try {
        await this.$store.dispatch('view/getViews', schemaID)
      } catch (err) {
        console.log(err)
      }
    },
    async deleteView(payload) {
      ...
      if (payload) {
        try {
          await this.$store.dispatch('view/deleteView', payload)
          this.getViews(this.schemaID)
        } catch (err) {
          console.log(err)
        }
