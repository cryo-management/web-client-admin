<template>
  <section>
    <div class="column">
      <nav class="breadcrumb" aria-label="breadcrumbs">
        <ul>
          ...
          <li class="is-active">
            <a href="#" aria-current="page">Lookup List</a>
          </li>
        </ul>
      </nav>
    </div>
    <div class="column card">
      ...
      <router-link class="button is-success" to="/admin/lookups/create">
        New Lookup
      </router-link>
...
export default {
  ...
  computed: {
    ...
    dataStore() {
      return this.$store.getters['lookup/lookups']
    },
  },
  watch: {
    dataStore() {
      this.data = this.$store.getters['lookup/lookups']
    },
  },
  mounted() {
    this.getLookups()
  ...
  methods: {
    async getLookups() {
      try {
        await this.$store.dispatch('lookup/getLookups')
      } catch (err) {
        console.log(err)
      }
    },
    async deleteLookup(payload) {
      this.isModalDelete = true
      if (payload) {
        try {
          await this.$store.dispatch('lookup/deleteLookup', payload.id)
          this.getLookups()
        } catch (err) {
          console.log(err)
        }
